          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-6">
            {popupData.map((album) => (
              <div
                ...
                />
                <div className="p-4">
                  <p className="mt-2 text-center flex justify-center items-center text-gray-800 font-bold">{album.title}</p>
                  {/* If the JSON has a single "song" object, display the file name without extension */}
                  <ul className="text-center mt-2 text-sm px-2 text-gray-800">
                    {album.song && (
                      <>
                        <li key={album.song.id} className="whitespace-normal break-words">
                          {album.song.file.replace(/\.\w+$/, "")}
                        </li>
                        <p className="mt-1 text-sm text-center text-gray-800">
                          {`(${(album.similarity_percentage * 100).toFixed(2)}%)`}
                        </p>
                      </>
                    )}
                  </ul>
