          ...
          <div className="search-results mt-8">
            ...
            <div className="grid grid-cols-2 sm:grid-cols-4 gap-10">
              {popupData.results.map((album) => (
                <div key={album.id} className="album-card w-48 cursor-pointer transition-transform transform hover:scale-105">
                  <img
                    src={`http://localhost:8000/static/${album.imageSrc.split("/").pop()}`}
                    alt={`Album ${album.title}`}
                    className="w-full h-48 object-cover rounded-lg"
                  />
                  <p className="mt-2 text-center flex justify-center items-center font-bold">
                    {album.title}
                    <span className="ml-2 text-sm text-gray-400">({album.similarity_percentage}%)</span>
                  </p>
                  {/* Insert the song list here */}
                  <ul className="mt-2 text-sm text-center px-2">
                    {album.songs &&
                      album.songs.map((song) => (
                        <li key={song.id} className="whitespace-normal break-words mt-1">
                          {song.file.replace(/\.\w+$/, "")}
                        </li>
                      ))}
                  </ul>
